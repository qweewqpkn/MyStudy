...
{
    //作用：挂在有text的对象上，实现文本颜色的渐变
    //问题：当和outline搭配使用的时候，要注意组件的顺序，先挂本脚本，后挂outline。 否则会出现报空的问题，因为outline如果先执行，会清空uv2和uv3导致我们这里PopulateUIVertex函数越界，Unity自身的问题，也可以解决，要下载ui源码进行修复
    [AddComponentMenu("UI/Effects/TextColorGradient")]
    public class TextColorGradient : BaseMeshEffect
    {
        public enum Type
        {
            Horizontal,
